...
def build_training_quality_mask_old(
    df: pd.DataFrame,
    vx_min_mps: float = 8.0,
    horz_acc_max_m: float = 15.0,
    abs_rxy_max_radps: float = 0.20,
    yaw_rxy_diff_max_radps: float = 0.15,
) -> pd.DataFrame:
    out = df.copy()

    out["ok_vx"] = (out["vx_mps"] >= vx_min_mps).astype(int)
    out["ok_yaw_reliable"] = (out["is_yaw_reliable"] >= 0.5).astype(int)
    out["ok_horz_acc"] = (out["horz_accuracy_m"] <= horz_acc_max_m).astype(int)
    out["ok_rxy"] = (np.abs(out["r_xy_radps"]) <= abs_rxy_max_radps).astype(int)
    out["ok_yaw_match"] = (
        np.abs(out["yaw_like_rate_radps"] - out["r_xy_radps"]) <= yaw_rxy_diff_max_radps
    ).astype(int)

    # 総合スコア
    out["quality_score"] = (
        1.5 * out["ok_vx"]
        + 1.5 * out["ok_yaw_reliable"]
        + 1.0 * out["ok_horz_acc"]
        + 1.0 * out["ok_rxy"]
        + 1.0 * out["ok_yaw_match"]
    )

    # 厳しめマスク
    out["quality_mask"] = (
        (out["ok_vx"] == 1)
        & (out["ok_yaw_reliable"] == 1)
        & (out["ok_horz_acc"] == 1)
        & (out["ok_rxy"] == 1)
        & (out["ok_yaw_match"] == 1)
    ).astype(int)
...
def find_train_val_test_time_windows(
    df: pd.DataFrame,
    outdir: Path,
    ...
    dfq = build_training_quality_mask(
        df,
        vx_min_mps=vx_min_mps,
    )
    dfq.to_csv(outdir / "quality_scored_dataframe.csv", index=False)

    seg_df = find_good_time_segments(dfq, min_duration_s=min_duration_s)
    seg_df.to_csv(outdir / "candidate_good_segments.csv", index=False)

    windows = propose_train_val_test_windows(seg_df, min_gap_s=min_gap_s)

    pd.DataFrame([
        {
            "split": "train",
            "t_start": windows["train_window"][0],
            "t_end": windows["train_window"][1],
        },
        {
            "split": "val",
            "t_start": windows["val_window"][0],
            "t_end": windows["val_window"][1],
        },
        {
            "split": "test",
            "t_start": windows["test_window"][0],
            "t_end": windows["test_window"][1],
        },
    ]).to_csv(outdir / "selected_time_windows.csv", index=False)
...
def compute_losses(
    ...
    """
    short-horizon rollout loss
    各時刻 t の truth state から horizon_steps だけ open-loop rollout し、
    その horizon 内の dX,dY,dPsi,dVx,r を比較する。
    """
    B, T, _ = can_seq.shape
    H = cfg.horizon_steps
    ...
    for s in range(n_start):
        ...
        loss_psi_list.append(loss_psi)
        loss_bicycle_list.append(loss_bicycle)
        loss_reg_list.append(loss_reg)

        beta_dot_res_all.append(beta_dot_res)
        r_dot_res_all.append(r_dot_res)
        dvx_res_all.append(dvx_res)

    loss_xy = torch.stack(loss_xy_list).mean()
    loss_v = torch.stack(loss_v_list).mean()
    loss_r = torch.stack(loss_r_list).mean()
    loss_psi = torch.stack(loss_psi_list).mean()
    loss_bicycle = torch.stack(loss_bicycle_list).mean()
    ...
    beta_dot_res_cat = torch.cat([x.reshape(-1) for x in beta_dot_res_all], dim=0)
    r_dot_res_cat = torch.cat([x.reshape(-1) for x in r_dot_res_all], dim=0)
    dvx_res_cat = torch.cat([x.reshape(-1) for x in dvx_res_all], dim=0)
...
def evaluate_model_on_windows(
    ...
    starts = np.arange(0, max(n - H + 1, 1), stride)
...
def train_vehicle_model(
    ...
    if auto_find_windows:
        windows = find_train_val_test_time_windows(
            df=df,
        ...
        if train_window is None or val_window is None or test_window is None:
            raise ValueError("auto_find_windows=False の場合は train_window, val_window, test_window を全て指定してください。")
    ...
    dfq = build_training_quality_mask(
        ...
        vx_min_mps=8.0,
#        horz_acc_max_m=15.0,
#        abs_rxy_max_radps=0.20,
#        yaw_rxy_diff_max_radps=0.15,
    )
    plot_quality_and_selected_windows(dfq, {
        "train_window": train_window,
        "val_window": val_window,
        "test_window": test_window,
    }, outdir / "window_selection")
    ...
    pd.DataFrame([
        {"split": "train", "t_start": train_window[0], "t_end": train_window[1], "n_rows": len(train_df)},
        {"split": "val", "t_start": val_window[0], "t_end": val_window[1], "n_rows": len(val_df)},
        {"split": "test", "t_start": test_window[0], "t_end": test_window[1], "n_rows": len(test_df)},
    ]).to_csv(outdir / "selected_windows_summary.csv", index=False)
